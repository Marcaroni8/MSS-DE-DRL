...
class DE(gym.Env):
    def __init__(self, fun, max_budget:int=1e4, state_features:bool=True, CR:float=1.0):
        ...
        self.observation_space = spaces.Box(-np.inf, np.inf, shape=(16+12*self.n_ops,), dtype = np.float32)
        
        self.NP = 100
        self.max_gen = 10
        self.number_metric = 4
        self.fun = fun
    ...
    def step(self, actions = []): # each step is one generation
        ...
        if self.state_features:
            ...
            for i in range(self.NP):
                if self.F1[i] <= self.F[i]:
                    ...
                    if self.F1[i] < np.median(self.F):
                        generation_metric[i,3] = np.median(self.F) - self.F1[i]
                    else:
                    ...
                    generation_metric[i,1:self.number_metric] = [-1] * 3
            ...
            ob[16:16+12*self.n_ops] = copy_ob
            ...
            ob[0] = normalise(self.pop_average, self.best_so_far, self.worst_so_far)
            ob[1] = self.pop_std / self.max_std
            ob[2] = self.budget / self.max_budget
            ob[3] = self.stagnation_count / self.max_budget
            
            ob = np.tile(ob, (self.NP,1))

            self.r = select_samples(self.NP, 5)
            for i in range(self.NP):
                ob[i,4:10] = distance.cdist(self.X[[self.r[i,0],self.r[i,1],self.r[i,2],self.r[i,3],self.r[i,4],self.best]], np.expand_dims(self.X[i], axis=0)).T / self.max_dist
                ob[i,10:16] = np.fabs(self.F[[self.r[i,0],self.r[i,1],self.r[i,2],self.r[i,3],self.r[i,4],self.best]] - self.F[i]) / (self.worst_so_far - self.best_so_far)
        else:
            ob = np.tile(ob, (self.NP,1))
    ...
    def reset(self):
        ...
        self.gen_window = [] # shape: (generations, NP, number_metric) or (generations, 100, 4)
